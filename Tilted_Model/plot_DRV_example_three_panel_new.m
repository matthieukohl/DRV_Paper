...
plot(x(indL:indR),heat_up(indL:indR)+rad_up(indL+indR),'b','linewidth',1.6);
%ylim([-max(qt_up(indL:indR)+10*max(abs(qt_low))) -min(qt_low(indL:indR)) ])
ylim([-max(qt_up(indL:indR)+10*max(abs(qt_low))) 0.012])
xlim([x(indL) x(indR)])
...
%saveas(gcf,'DRV_Paper/Figures/DRV_mode_example_three_panel','epsc')
