...
for ii = 1:length(R) %length(R) %length(R)
 ...
 for jj = 1:length(h) %length(h) %length(h):length(h) 
     tic
ii
jj
...
% save('/disk7/mkohl/Mode_Kohl_OGorman_beta_no_gradients/mode_or_drv/mode_or_drv_N300_L12pi_t400_new.mat','L','N','t_final','R','h','ww','sigma','sigma2',...
%   'b','mode_or_drv')
